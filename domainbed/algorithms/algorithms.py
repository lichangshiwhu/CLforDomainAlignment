...
class ERM(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        ...
        self.trans = nn.Linear(self.featurizer.n_outputs, self.embedding, bias=False)
        self.classifier = nn.Linear(self.embedding, num_classes, bias=False)
        torch.nn.init.orthogonal_(self.trans.weight)
        torch.nn.init.orthogonal_(self.classifier.weight)
        self.network = nn.Sequential(self.featurizer, self.trans, self.classifier)
...
class SCL(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(SCL, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.scale = hparams["scale"]
        self.alpha = hparams["alpha"]
        self.beta = hparams["beta"]
        self.loss_name = "cl_w"
        # if self.num_classes < self.hparams["batch_size"] * (num_domains - 1):
        #     self.loss_name = "cl_w"
        # else:
        #     self.loss_name = "cl_s"
    def contrastive_loss_w(self, feature, target):
        feature = F.normalize(feature, p=2, dim=1)
        weights = self.classifier.weight.data.clone().detach()
        # weights = self.classifier.weight.data.clone()
        pred = F.linear(feature, F.normalize(weights, p=2, dim=1))  # (N, C)
    ...
    def contrastive_loss_s(self, feature, target):
        feature = F.normalize(feature, p=2, dim=1)
        weights = self.classifier.weight.data.clone().detach()
        # weights = self.classifier.weight.data.clone()
        pred = F.linear(feature, F.normalize(weights, p=2, dim=1))  # (N, C)
    ...
    def post_mlp_anchor_confusion_loss_normalize(self, feature, target):
        feature = F.normalize(feature, p=2, dim=1)
        weights = self.classifier.weight.data.clone().detach()
        pred = F.linear(feature, F.normalize(weights, p=2, dim=1))  # (N, C)
        feature = torch.matmul(feature, feature.transpose(1, 0))  # (N, N)
    ...
    def regularize_trans_loss(self):
        weights = self.trans.weight
        weights = F.normalize(weights, p=2, dim=1) # (N1, N)
        weights = torch.matmul(weights, weights.transpose(1, 0))
        labels = torch.arange(weights.shape[0], dtype=torch.long).to(weights.device)
        return F.nll_loss(F.log_softmax(weights, dim=1), labels)

    def update(self, x, y, **kwargs):
        all_x = torch.cat(x)
        all_y = torch.cat(y)
        # last layer is relu, which make featurizer > 0 
        # and can't converage to a better result.
        featurizer = self.featurizer(all_x)
        # get more free featurizer
        featurizer = self.trans(featurizer)
        classifier = self.classifier(featurizer)
        # caculate number of classes in featurizer layer of dim cluster
        contrastive_loss = 0
        if self.loss_name == "cl_w":
            contrastive_loss = self.contrastive_loss_w(featurizer, all_y)
        else:
            contrastive_loss = self.contrastive_loss_s(featurizer, all_y)
        regularize_trans_loss = self.regularize_trans_loss()
        ce_loss = F.cross_entropy(classifier, all_y)
        loss = self.alpha * contrastive_loss + ce_loss  + self.beta * regularize_trans_loss
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        return {"ce_loss": ce_loss.item(), self.loss_name: contrastive_loss.item(), 
        "regularize_trans_loss": regularize_trans_loss.item()}

class GCL(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(GCL, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.scale = hparams["scale"]
        self.alpha = hparams["alpha"]
        self.beta = hparams["beta"]
    def post_mlp_anchor_normalize(self, feature, target):
        feature = F.normalize(feature, p=2, dim=1)
        weights = self.classifier.weight.data.clone().detach()
        pred = F.linear(feature, F.normalize(weights, p=2, dim=1))  # (N, C)
        feature = torch.matmul(feature, feature.transpose(1, 0))  # (N, N)
    ...
    def update(self, x, y, **kwargs):
        ...
        featurizer = self.trans(featurizer)
        classifier = self.classifier(featurizer)
        global_contrastive_loss = self.post_mlp_anchor_normalize(featurizer, all_y)
        regularize_trans_loss = self.regularize_trans_loss()
        ce_loss = F.cross_entropy(classifier, all_y)
        loss = self.alpha * global_contrastive_loss + ce_loss + self.beta * regularize_trans_loss
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        return {"ce_loss": ce_loss.item(), "GCL_loss": global_contrastive_loss.item(), "regularize_trans_loss": regularize_trans_loss.item()}
